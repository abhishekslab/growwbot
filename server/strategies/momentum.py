...
def _dbg(msg: str, data: dict, hyp: str) -> None:
    try:
        ...
        with open(_DBG_LOG, "a") as _f:
            _f.write(
                json.dumps({"timestamp": int(time.time() * 1000), "location": "algo_momentum.py", "hypothesisId": hyp, "message": msg, "data": data})
                + "\n"
            )
    except Exception:
        pass
...
class MomentumScalping(BaseAlgorithm):
    ...
    def evaluate(self, symbol: str, candles: List[dict], ltp: float, candidate_info: dict) -> Optional[AlgoSignal]:
        ...
        if target_move <= required_move:
            ...
            _dbg(
                "Fee margin rejected",
                {"target_move": target_move, "required_move": required_move, "fee_breakeven": fee_breakeven, "qty": quantity, "atr": atr},
                "H-F",
            )
            return None
        ...
        reason = "EMA %d/%d crossover, RSI %.1f, Vol %.1fx, Above VWAP %.2f, ATR %.2f, Target +%.2f (%.1fx ATR)" % (
            ema_fast,
            ema_slow,
            rsi,
            vol["ratio"],
